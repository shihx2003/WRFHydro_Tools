...
# Output Directory
OutDir = r'F:/Geo_For_GIS/Domain/Zijinguan2'  # Output Directory

# Input netCDF GEOGRID file
isGEOGRID = True                                                               # Is the desired domain a GEOGRID file?
in_geogrid = r"F:\Geo_For_GIS\Domain\Zijinguan2\geo_em.d03.nc"
geogridVariable = 'HGT_M'

# Input vector layer (basins, etc)
inDriverName = 'ESRI Shapefile'                                                 # 'GPKG' / 'ESRI Shapefile'
in_basins = r"F:\Geo_For_GIS\Domain\Zijinguan2\catchments.shp"
layerName = 'Basins'
...
use_inRaster = False
if use_inRaster:
    in_nc = r"F:\Geo_For_GIS\Domain\Zijinguan2\Zijinguan.tif"
    Variable = 'HGT_M'
    nest = 1
elif isGEOGRID:
...
def Read_GEOGRID_for_SRS(in_nc):
    ...
    wgs84_proj = osr.SpatialReference()
    wgs84_proj.ImportFromEPSG(4326)
    # ===== 终极修复：使用 PROJ4 字符串代替 EPSG 查库 =====
    wgs84_proj.ImportFromProj4('+proj=longlat +datum=WGS84 +no_defs')

    import osgeo
    if int(osgeo.__version__[0]) >= 3:
        wgs84_proj.SetAxisMappingStrategy(osgeo.osr.OAMS_TRADITIONAL_GIS_ORDER)
        proj1.SetAxisMappingStrategy(osgeo.osr.OAMS_TRADITIONAL_GIS_ORDER)

    transform = osr.CoordinateTransformation(wgs84_proj, proj1)
    
    if transform is None:
        print("【致命错误】：坐标转换器初始化失败，请检查投影定义！")
        sys.exit(1)
    # =================================================
    # transform = osr.CoordinateTransformation(wgs84_proj, proj1)
    point = ogr.Geometry(ogr.wkbPoint)
